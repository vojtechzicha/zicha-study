...
export function StudyNotesDisplaySection({
  ...
  study,
  isPublicView = false,
  showSubtitle: _showSubtitle = false,
  ...
  const loadStudyNotes = useCallback(async (silent = false) => {
    ...
    try {
      const notesData = await fetchStudyNotes(studyId, isPublicView) as RawStudyNoteFromMongo[]
  ...
  }, [studyId, isPublicView, showSubjectNames])

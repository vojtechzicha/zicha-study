...
export function GlobalTaskRow({ task, study, onClick, compact = false }: GlobalTaskRowProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "group flex items-start gap-2.5 rounded-lg border bg-card px-2.5 py-2 transition-all cursor-pointer",
        "hover:shadow-sm hover:-translate-y-0.5",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-1",
        state === "overdue" && "border-red-200 bg-red-50/40 dark:border-red-800 dark:bg-red-950/30",
        state === "running" && "border-amber-200 dark:border-amber-800",
        state === "upcoming" && "border-primary-200 dark:border-primary-800",
        state === "completed" && "border-green-200 bg-green-50/30 opacity-80 dark:border-green-800 dark:bg-green-950/30",
      )}
    >
      <StudyLogo
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[11px] text-muted-foreground">{study.name}</p>
      </div>
    </div>
  )
}
